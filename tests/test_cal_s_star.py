...
def test_phased_ind2_hap1_emits_late_windows_as_na(data):
    """
    Regression test: ensure phased output includes ind2 hap1 rows for
    30000-80000 and 40000-90000 (as NA rows when low-information).
    """
    output_phased = "./tests/results/test.score.phased_same_input.results"
    ...
    rows = []
    with open(output_phased, "r") as f:
        header = f.readline().rstrip("\n").split("\t")
        col = {name: i for i, name in enumerate(header)}
        for line in f:
            e = line.rstrip("\n").split("\t")
            if e[col["sample"]] == "ind2" and e[col["hap_index"]] == "1":
                rows.append(e)

    def find_row(start, end):
        for r in rows:
            if r[col["start"]] == str(start) and r[col["end"]] == str(end):
                return r
        return None

    r1 = find_row(30000, 80000)
    r2 = find_row(40000, 90000)

    assert r1 is not None, "Missing phased row for ind2 hap1 window 30000-80000"
    assert r2 is not None, "Missing phased row for ind2 hap1 window 40000-90000"

    # Expected NA S* rows for sparse windows
    assert r1[col["S*_score"]] == "NA"
    assert r1[col["S*_SNP_number"]] == "NA"
    assert r1[col["S*_SNPs"]] == "NA"

    assert r2[col["S*_score"]] == "NA"
    assert r2[col["S*_SNP_number"]] == "NA"
    assert r2[col["S*_SNPs"]] == "NA"
