...
def cal_threshold(
    ...
    """
    Calculate significant S* thresholds for empirical score windows.

    Parameters
    ----------
    simulated_data : str
        Path to the simulated quantile summary file.
    score_file : str
        Path to the score file generated by `sstar score`.
    recomb_rate : float
        Uniform recombination rate used when no recombination map is provided.
    recomb_map : str or None
        Path to the recombination-map file. If None, use `recomb_rate` when local recombination is needed.
    quantile : float
        Quantile used to estimate significant S* scores.
    output : str
        Path to the output threshold file.
    k : int
...
def _build_gam_model(simulated_data: str, k: int, fit_lr: bool) -> object:
    """
    Build an MGCV generalized additive model for expected S* scores.
    ...
    k : int
        Basis dimension passed to the MGCV smooth term.
    fit_lr : bool
        If True, include local recombination rate in the model.

    Returns
    -------
    object
        Fitted MGCV GAM model exposed through rpy2.
    """
    mgcv = importr("mgcv")
...
def _predict_res(
    ...
    """
    Predict expected S* scores and format threshold output rows.

    Parameters
    ----------
    gam : object
        Fitted MGCV GAM model.
    score_file : str
        Path to the score file generated by `sstar score`.
    recomb_rate : float
        Uniform recombination rate used when no recombination map is provided.
    recomb_map : str or None
        Path to the recombination-map file. If None, use `recomb_rate` when needed.
    quantile : float
        Quantile used to estimate significant S* scores.
    fit_lr : bool
    ...
    -------
    list
        Formatted output lines containing observed scores, expected scores, and significance flags.
    """
    stats = importr("stats")
...
def _read_score_file(
    score_file: str,
    recomb_rate: float,
    recomb_map: Optional[dict],
    quantile: float,
    fit_lr: bool,
    phased: bool,
) -> Tuple[dict, dict]:
    """
    Read S* score rows and prepare prediction inputs for thresholding.
    ...
    recomb_rate : float
        Uniform recombination rate used when no recombination map is provided.
    recomb_map : dict or None
        Local recombination rates keyed by `chrom:start-end`. If None, use `recomb_rate` when needed.
    quantile : float
        Quantile used to estimate significant S* scores.
    fit_lr : bool
        If True, include local recombination rate in prediction data.
    ...
    -------
    tuple
        Tuple containing prediction data by sample and metadata rows by sample.
    """
    data = {}
    meta_data = {}
...
def _read_recomb_map(recomb_map_file: str) -> dict:
    """
    Read a recombination-map file.

    Parameters
    ----------
    recomb_map_file : str
        Path to the recombination-map file.

    Returns
    -------
    dict
        Local recombination rates keyed by `chrom:start-end`.
    """
    recomb_map = {}
