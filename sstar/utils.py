...
def parse_ind_file(filename):
    ...
    try:
        ...
        if len(samples) == 0:
            raise ValueError(f"No sample is found in {filename}! Please check your data.")
...
def read_geno_data(vcf, ind, anc_allele_file, filter_missing):
    ...
    ind : list
        List containing names of samples.
    anc_allele_file : str
        Name of the BED file containing ancestral allele information.
    filter_missing : bool
        Indicating whether filtering missing data or not.
    ...
    """
    try:
        vcf_data = allel.read_vcf(vcf, alt_number=1, samples=ind)
        if vcf_data is None:
            raise ValueError(f"Failed to read VCF file {vcf}")
            
        gt = vcf_data["calldata/GT"]
        chr_names = np.unique(vcf_data["variants/CHROM"])
    ...
    for c in chr_names:
        if c not in data.keys():
            ...
            data[c]["ALT"] = alt
            data[c]["GT"] = gt
        index = np.where(vcf_data["variants/CHROM"] == c)
...
def filter_data(data, c, index):
    ...
    index : numpy.ndarray
        A boolean array determines variants to be kept.
...
def read_data(vcf, ref_ind_file, tgt_ind_file, src_ind_file, anc_allele_file):
    ...
    ----------
    vcf : str
        Name of the VCF file containing genotype data from reference, target, and source populations.
    ref_ind_file : str
...
def get_ref_alt_allele(ref, alt, pos):
    ...
    alt_allele : dict
        ALT alleles.
    """
    ref_allele = {p: r for p, r in zip(pos, ref)}
    alt_allele = {p: a for p, a in zip(pos, alt)}
...
def read_anc_allele(anc_allele_file):
    ...
    ValueError
        If no ancestral allele information is found.
    """
    try:
        anc_allele = dict()
        ...
        if not anc_allele:
            raise ValueError(f"No ancestral allele is found! Please check your data.")
...
def check_anc_allele(data, anc_allele, c):
    ...
    anc_allele : dict
        Ancestral allele information for checking.
    c : str
    ...
    intersect_snps = np.intersect1d(list(ref_allele.keys()), list(anc_allele.get(c, {}).keys()))
    ...
    # Flip variants where ALT is the ancestral allele
...
def read_mapped_region_file(mapped_region):
    ...
    ----------
    mapped_region : str
        BED file containing mapped regions.

    Returns
    -------
    mapped_intervals : dict
        Dictionary of tuples containing mapped regions.
    """
    if mapped_region != None:
        try:
            ...
            raise FileNotFoundError(f"Mapped region file {mapped_region} not found")
    else:
        return None
...
def cal_matchpct(
    ...
    data : dict
        Genotype data from individuals to be calculated match percents.
    src_data : dict
        Genotype data from source populations.
    tgt_ind_index : int
        Index of the target individual for calculating match percent.
    src_ind_index : int
        Index of the source individual for calculating match percent.
    hap_index : int
        Index of the haplotype for calculating match percent.
    win_start : int
        Start position of the window for calculating match percent.
    win_end : int
        End position of the window for calculating match percent.
    sample_size : int
    ...
    -------
    res : list
        List containing statistics for the given haplotype.
    """
...
def _cal_mapped_len(mapped_intervals, chr_name, win_start, win_end):
    ...
    mapped_intervals : dict
        Dictionary of tuples containing mapped regions across the genome.
    chr_name : str
        Name of the chromosome for calculating the length of the mapped region.
    win_start : int
        Start position of a window.
    win_end : int
        End position of a window.
...
def _cal_hap_stats(
    ...
    gt : allel.GenotypeArray
        Genotype data for all the haplotypes within the same window of the haplotype to be analyzed.
    hap : allel.GenotypeVector
        Genotype data for the haplotype to be analyzed.
    pos : list
        List containing positions of variants on the haplotype.
    src_variants : list
        List containing positions of variants on the individual from the source population.
    src_hom_variants : list
        List containing positions of homozygous variants on the individual from the source population.
    src_het_variants : list
        List containing positions of heterozygous variants on the individual from the source population.
    sample_size : int
        Number of individuals analyzed.

    Returns
    -------
    hap_variants_num : int
        Number of SNPs with derived alleles on the haplotype.
    hap_site_num : int
        Number of SNPs with derived alleles either on the haplotype or the source genomes.
    hap_match_src_allele_num : int
        Number of SNPs with derived alleles both on the haplotype and the source genomes.
    hap_sfs : int
        Average number of derived variants per site per haplotype.
    hap_match_pct : float
        Match percent of the haplotype.
    """
    if hap is None:
...
def py2round(x, d=0):
    """
    Python 2-style rounding function.
...
def split_genome(pos, window_size, step_size, start=None):
    """
    ...
    start: int, optional
        Minimum starting coordinate for the first window.
